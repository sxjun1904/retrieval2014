	...
	<c:if test="${menu.parentid eq (not empty param.parentId?param.parentId:1)&&menu.isShow eq 1}">
		<div class="accordion-group">
		    <div class="accordion-heading">
		    	...
		    	<i class="icon-chevron-${firstMenu?'down':'right'}"></i>&nbsp;${menu.name}</a>
		    </div>
					...
					<ul class="nav nav-list">
						<c:forEach items="${menuList}" var="menuChild">
							<c:if test="${menuChild.parentid eq menu.id&&menuChild.isShow eq 1}">
								<li>
									<a href="${fn:indexOf(menuChild.href, '://') eq -1?ctx_a:''}${not empty menuChild.href?menuChild.href:'/404'}" target="${not empty menuChild.target?menuChild.target:'mainFrame'}" >
