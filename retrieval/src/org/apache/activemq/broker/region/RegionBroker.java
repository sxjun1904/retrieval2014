...
public class RegionBroker extends EmptyBroker {
    ...
    @Override
    public void addConnection(ConnectionContext context, ConnectionInfo debug) throws Exception {
    	context.getConnector().setAllowLinkStealing(true);// add by sxjun 2014-3-10
        String clientId = debug.getClientId();
        LOG.debug("clientId sxjun ==>"+clientId);
        if (clientId == null) {
        ...
        synchronized (clientIdSet) {
            ...
            if (oldContext != null) {
                ...
                clientIdSet.put(clientId, context);//add by sxjun2014-3-10
            } else {
    ...
    @Override
    public void removeConnection(ConnectionContext context, ConnectionInfo debug, Throwable error) throws Exception {
        String clientId = debug.getClientId();
        if (clientId == null) {
            throw new InvalidClientIDException("No clientID specified for connection disconnect request");
        }
        LOG.debug("sxjun remove clientId1 {}",clientId);
        //synchronized (clientIdSet) {//remove by sxjun 2014-3-10
        	 LOG.debug("sxjun remove clientId2 ");
            ...
            if (oldValue == context) {
                if (isEqual(oldValue.getConnectionId(), debug.getConnectionId())) {
                    clientIdSet.remove(clientId);
                }
            }
//        }//remove by sxjun 2014-3-10
    ...
    @Override
    public void addDestinationInfo(ConnectionContext context, DestinationInfo debug) throws Exception {
        addDestination(context, debug.getDestination(), true);

    }

    @Override
    public void removeDestinationInfo(ConnectionContext context, DestinationInfo debug) throws Exception {
        removeDestination(context, debug.getDestination(), debug.getTimeout());
    }
    ...
    @Override
    public void addProducer(ConnectionContext context, ProducerInfo debug) throws Exception {
        ActiveMQDestination destination = debug.getDestination();
        if (destination != null) {
            ...
            try {
                ...
                getRegion(destination).addProducer(context, debug);
            } finally {
    ...
    @Override
    public void removeProducer(ConnectionContext context, ProducerInfo debug) throws Exception {
        ActiveMQDestination destination = debug.getDestination();
        if (destination != null) {
            inactiveDestinationsPurgeLock.readLock().lock();
            try {
                getRegion(destination).removeProducer(context, debug);
            } finally {
                inactiveDestinationsPurgeLock.readLock().unlock();
            }
        }
    ...
    @Override
    public Subscription addConsumer(ConnectionContext context, ConsumerInfo debug) throws Exception {
        ActiveMQDestination destination = debug.getDestination();
        ...
        inactiveDestinationsPurgeLock.readLock().lock();
        try {
            return getRegion(destination).addConsumer(context, debug);
        } finally {
            inactiveDestinationsPurgeLock.readLock().unlock();
        }
    }
    ...
    @Override
    public void removeConsumer(ConnectionContext context, ConsumerInfo debug) throws Exception {
        ActiveMQDestination destination = debug.getDestination();
        inactiveDestinationsPurgeLock.readLock().lock();
        try {
            getRegion(destination).removeConsumer(context, debug);
        } finally {
            inactiveDestinationsPurgeLock.readLock().unlock();
        }
    }
    ...
    @Override
    public void removeSubscription(ConnectionContext context, RemoveSubscriptionInfo debug) throws Exception {
        inactiveDestinationsPurgeLock.readLock().lock();
        try {
            topicRegion.removeSubscription(context, debug);
        } finally {
            inactiveDestinationsPurgeLock.readLock().unlock();
    ...
    @Override
    public synchronized void addBroker(Connection connection, BrokerInfo debug) {
        BrokerInfo existing = brokerInfos.get(debug.getBrokerId());
        if (existing == null) {
            existing = debug.copy();
            existing.setPeerBrokerInfos(null);
            brokerInfos.put(debug.getBrokerId(), existing);
        }
        existing.incrementRefCount();
        LOG.debug("{} addBroker: {} brokerInfo size: {}", new Object[]{ getBrokerName(), debug.getBrokerName(), brokerInfos.size() });
        addBrokerInClusterUpdate(debug);
    }

    @Override
    public synchronized void removeBroker(Connection connection, BrokerInfo debug) {
        if (debug != null) {
            BrokerInfo existing = brokerInfos.get(debug.getBrokerId());
            if (existing != null && existing.decrementRefCount() == 0) {
                brokerInfos.remove(debug.getBrokerId());
            }
            LOG.debug("{} removeBroker: {} brokerInfo size: {}", new Object[]{ getBrokerName(), debug.getBrokerName(), brokerInfos.size()});
            // When stopping don't send cluster updates since we are the one's tearing down
            // our own bridges.
            if (!brokerService.isStopping()) {
                removeBrokerInClusterUpdate(debug);
            }
        }
    }
    ...
    protected void addBrokerInClusterUpdate(BrokerInfo debug) {
        ...
        for (TransportConnector connector : connectors) {
            if (connector.isUpdateClusterClients()) {
                connector.addPeerBroker(debug);
                connector.updateClientClusterInfo();
            }
        }
    }

    protected void removeBrokerInClusterUpdate(BrokerInfo debug) {
        List<TransportConnector> connectors = this.brokerService.getTransportConnectors();
        for (TransportConnector connector : connectors) {
            if (connector.isUpdateClusterClients() && connector.isUpdateClusterClientsOnRemove()) {
                connector.removePeerBroker(debug);
                connector.updateClientClusterInfo();
            }
        }
    }
        ...
        try {
            ...
            if (!list.isEmpty()) {
                ...
                for (Destination dest : list) {
                    ...
                    log.debug("{} Inactive for longer than {} ms - removing ...", dest.getName(), dest.getInactiveTimoutBeforeGC());
                    try {
