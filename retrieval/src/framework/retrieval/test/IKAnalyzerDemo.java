package framework.retrieval.test;
...
public class IKAnalyzerDemo { 
	public static void main(String[] args){
		...
		try { 
			//建立内存索引对象
			directory = FSDirectory.open(new File("D:\\apache-tomcat-6.0.21\\index\\BS\\DB\\TEST_WEB"));
			//配置IndexWriterConfig 
			...
			isearcher = new IndexSearcher(ireader); String keyword = "城镇化"; 
			...
			{ e.printStackTrace(); } 
			finally{ if(ireader != null){ 
				try { ireader.close(); } 
				catch (IOException e) 
				{ e.printStackTrace(); } } 
			if(directory != null)
			{ try { directory.close(); } catch (IOException e) { e.printStackTrace(); } } } }}