...
public class BasicCrawlController
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		/*
		 * Do you need to set a proxy? If so, you can use: config.setProxyHost("proxyserver.example.com"); config.setProxyPort(8080); If your proxy
		 * also needs authentication: config.setProxyUsername(username); config.getProxyPassword(password);
		 */

		/*
		 * This config parameter can be used to set your crawl to be resumable (meaning that you can resume the crawl from a previously
		 * interrupted/crashed crawl). Note: if you enable resuming feature and want to start a fresh crawl, you need to delete the contents of
		 * rootFolder manually.
		 */
		...
		/*
		 * For each crawl, you need to add some seed urls. These are the first URLs that are fetched and then the crawler starts following links which
