...
package frame.crawler4j.url;

public final class UrlResolver
{

	/**
	 * Resolves a given relative URL against a base URL. See <a href="http://www.faqs.org/rfcs/rfc1808.html">RFC1808</a> Section 4 for more details.
	 * 
	 * @param baseUrl
	...
	/**
	 * Parses a given specification using the algorithm depicted in <a href="http://www.faqs.org/rfcs/rfc1808.html">RFC1808</a>: Section 2.4: Parsing
	 * a URL An accepted method for parsing URLs is useful to clarify the generic-RL syntax of Section 2.2 and to describe the algorithm for resolving
	 * relative URLs presented in Section 4. This section describes the parsing rules for breaking down a URL (relative or absolute) into the
	 * component parts described in Section 2.1. The rules assume that the URL has already been separated from any surrounding text and copied to a
	 * "parse string". The rules are listed in the order in which they would be applied by the parser.
	 * 
	 * @param spec
	...
	/**
	 * Resolves a given relative URL against a base URL using the algorithm depicted in <a href="http://www.faqs.org/rfcs/rfc1808.html">RFC1808</a>:
	 * Section 4: Resolving Relative URLs This section describes an example algorithm for resolving URLs within a context in which the URLs may be
	 * relative, such that the result is always a URL in absolute form. Although this algorithm cannot guarantee that the resulting URL will equal
	 * that intended by the original author, it does guarantee that any valid URL (relative or absolute) can be consistently transformed to an
